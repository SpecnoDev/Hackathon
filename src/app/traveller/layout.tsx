...
import { currentTravellerOrDemo, forbidHostOnTravellerRoutes } from '@/core/guards';
...
export default async function TravellerLayout({ children }: { children: ReactNode }) {
  ...
  const user = await currentTravellerOrDemo();
  const traveller = user ? { name: user.traveller.name } : null;
