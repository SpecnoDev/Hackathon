...
import { openTimes } from '../services/client';
...
export const TimeSlots = ({ times, date, selected, onSelect }: TimeSlotsProps) => {
  ...
  if (open.length === 0) return <p className="text-body-md text-body">{times.length === 0 ? COPY_COMMON.anyTime : copy.noTimesLeft}</p>;

  return (
    <div role="group" aria-label={copy.timeTitle} className="flex flex-wrap gap-2">
      {open.map((time) => (
